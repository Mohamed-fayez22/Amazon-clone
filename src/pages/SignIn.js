...
const SignIn = () => {
  ...
  // firebase Error
  const [emailUserErr, setUserEmailErr] = useState("");
  const [passwordUserErr, setUserPassErr] = useState("");
  // loading State
  ...
  const handleLogin = (e) => {
    ...
    if (Emaill && Passward) {
      setLoading(true);
      signInWithEmailAndPassword(auth, Emaill, Passward)
        .then((userCredential) => {
          // Signed in
        ...
        .catch((error) => {
          ...
          if (errorCode.includes("auth/wrong-password")) {
            setUserPassErr("WrongPassword! Try again");
          }
            ...
            {sucssMsg && (
              <div className="flex justify-center items-center">
                <motion.p
                  ...
                  className=" border-[1px] border-green-500 text-green-500 text-center font-titleFont font-semibold
                    px-6 my-2 py-2 text-lg"
                >
                  {sucssMsg}
                </motion.p>
              </div>
          ...
          <form className="w-[360px] mx-auto flex flex-col items-center">
            ...
            <div className="w-full border border-zinc-200 p-6">
              ...
              <p className="text-xs text-black leading-4 mt-4">
                ...
                <span className=" text-blue-600 group">Privace Notice.</span>
              </p>
              <p className="text-xs mt-5 text-gray-600 cursor-pointer group">
                <ArrowRightIcon />
                <span
                  className="text-xs text-blue-600 group-hover:text-orange-700 
            ...
            <p className="w-full text-xs text-gray-600 mt-4 flex items-center ">
              <span className="h-[1px] w-1/3 bg-zinc-400 inline-flex"> </span>
            ...
            <Link className="w-full" to="/regestration">
              <button
                ...
                className="  w-full text-sm py-1.5 mt-4 font-normal rounded-sm bg-gradient-to-t from-slate-200 to-slate-100 hover:bg-gradient-to-b 
          border border-zinc-400 active:hover-yellow-800 active:shadow-amazonInput "
              >
